<script lang="ts">
	...
	$: availableClass = data.availableClass.reduce((res, curr) => {
			...
			{#each availableClass as cc, i}
				<section class="flex-col flex">
					<h2
						class="{bg[
							i
						]} text-center text-xl md:text-3xl capitalize rounded-t-md px-2 ring-1 ring-black"
					>
						kelas {i + 10}
					</h2>
					<ul class="flex flex-col bg-white rounded-b-md ring-1 ring-black py-1 h-full">
						{#each cc as c}
							<li class="w-full px-2">
								- <a href="/jadwal/{c.replace(/\s/gi, '-')}" class="underline text-blue-500">
									{c}
								</a>
							</li>
						{/each}
