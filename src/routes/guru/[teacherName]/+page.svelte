...
<section class="p-2 flex flex-col gap-8">
	<div class="relative select-none w-full md:w-1/2 mx-auto">
		<div class="ring-1 ring-black h-full w-full z-10 relative bg-sky-300 p-2 flex flex-col"> 
			<h1 class="text-3xl md:text-5xl text-center font-bold">Jadwal {$page.params.teacherName.replace(/-/gi, " ")}</h1>
			<form 
		...
		<div class="absolute -top-2 -left-2 bg-blue-600 ring-1 ring-black h-full w-full"></div>
	</div>
	{#if simple}
		<SimpleSchedules {currentDate} schedules={data.schedules} displayText={s => s.class}/>
	{:else}
		<div class="relative select-none w-full md:w-1/2 mx-auto">
			<div class="w-full relative z-10">
				<FullSchedules {currentDate} schedules={data.schedules} displayText={s => `${s.class} (${s.name})`} />
			</div>
			<div class="absolute -top-2 -left-2 bg-blue-600 ring-1 ring-black h-full w-full"></div>
		</div>
	{/if}
