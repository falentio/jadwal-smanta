...
{#if showNav || showMenu}
	<div class="absolute w-full h-full z-20 p-4 pointer-events-none v">
		<nav class="w-full sticky top-4 ">
			{#if showMenu}
				<div
					class="w-screen bg-zinc-900/90 top-0 left-0 z-[500] fixed text-sky-500 pointer-events-auto"
				>
					<section
						class="w-full min-h-screen sticky top-0 mx-auto flex-col flex items-center pt-24"
			...
			{:else}
				<div class="max-w-3xl mx-auto justify-end flex flex-row">
					<button
						class="bg-sky-300 border border-black w-12 h-12 flex justify-center items-center"
