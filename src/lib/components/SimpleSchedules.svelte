	...
	{#each weekday as i (i)}
		...
		<div class="w-1/2 md:w-1/3 shrink-0 md:text-xl flex flex-col">
			<section class="m-1 p-2 h-full rounded-md flex flex-col">
				<h2 class="text-xl md:text-3xl font-bold bg-sky-500 w-max px-2 rounded-md pb-8 shadow-xl">
				...
				<ul class="divide-y divide-black bg-sky-300 p-2 ml-2 -mt-8 rounded-md h-full">
					{#each schedule as s, i (s)}
						<li class="text-sm md:text-md grid grid-cols-[auto_1fr] gap-2 auto-cols-max">
